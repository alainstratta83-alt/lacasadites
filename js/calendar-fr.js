...
// ====================================
const ADVANCE_NOTICE_DAYS = 5; // 🔧 MODIFIER CE NOMBRE POUR CHANGER LE PRÉAVIS
...
class BookingCalendar {
    ...
    async loadOccupiedDates() {
        try {
            ...
            if (response.ok) {
                ...
                console.log('📅 Dates chargées depuis le serveur:', dates.length);
    ...
    saveOccupiedDates() {
        ...
        this.showNotification('✅ Disponibilité enregistrée!', 'success');
    }
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (currentDayDate < minBookableDate) {
                ...
                if (currentDayDate >= today && currentDayDate < minBookableDate) {
                    dayElement.title = `Préavis de ${this.advanceNoticeDays} jours requis`;
                }
            } else {
        ...
        if (this.adminMode) {
            ...
            if (date < minBookableDate) {
                ...
                if (daysUntil < this.advanceNoticeDays && daysUntil >= 0) {
                    this.showNotification(`⏰ Préavis de ${this.advanceNoticeDays} jours requis. Cette date est trop proche (${daysUntil} jours)`, 'error');
                } else {
                    this.showNotification('❌ Date dans le passé', 'error');
                }
                return;
            }
            ...
            if (isOccupied) {
                this.showNotification('❌ Cette date n\'est pas disponible', 'error');
                return;
            }
            ...
            if (!this.selectedStartDate || (this.selectedStartDate && this.selectedEndDate)) {
                ...
                if (nights < this.minimumNights) {
                    this.showNotification(`⚠️ Séjour minimum requis: ${this.minimumNights} nuits. Vous avez sélectionné ${nights} nuit(s).`, 'error');
                    this.selectedStartDate = null;
                    this.selectedEndDate = null;
                    this.renderCalendar();
                    this.updateBookingInfo();
                ...
                if (this.hasOccupiedInRange(this.selectedStartDate, this.selectedEndDate)) {
                    this.showNotification('❌ Il y a des dates occupées dans la période sélectionnée', 'error');
                    this.selectedStartDate = null;
                    this.selectedEndDate = null;
                }
            }
    ...
    setupEventListeners() {
        ...
        document.getElementById('bookNow').addEventListener('click', () => {
            if (this.selectedStartDate && this.selectedEndDate) {
                ...
                this.showNotification('✅ Dates sélectionnées! Remplissez le formulaire ci-dessous', 'success');
            }
        });
        ...
        if (toggleAdmin) {
            toggleAdmin.addEventListener('click', () => {
                ...
                if (this.adminMode) {
                    indicator.textContent = '🔧 MODE ADMIN ACTIF';
                    indicator.style.background = '#FFE69C';
                    adminPanel.style.border = '3px solid #FF9800';
                    this.showNotification('🔧 Mode Admin: Cliquez sur les jours pour modifier la disponibilité', 'info');
                } else {
                    indicator.textContent = '👤 Mode Invité';
        ...
        if (clearBtn) {
            clearBtn.addEventListener('click', () => {
                if (confirm('Êtes-vous sûr de vouloir effacer TOUTES les dates?')) {
    ...
    exportCalendar() {
        ...
        link.href = url;
        link.download = `calendrier-casa-tes-${new Date().toISOString().split('T')[0]}.json`;
        link.click();
        
        URL.revokeObjectURL(url);
        this.showNotification('📥 Calendrier exporté!', 'success');
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('calendarDays')) {
        ...
        console.log('📅 Calendrier de réservation initialisé');
    }
});
